...
6. Users can choose the request format (`openai-compatible` or `gemini-native`), fill request API base / API key, and override the code model and image model per job
...
Make sure that file contains the server-side fallback API settings you actually want to use. The app can also override provider / base / models per job from the frontend, but the server-side config remains the fallback template.
...
window.SLIDE_APP_CONFIG = {
  ...
  requestProvider: "openai-compatible",
  openaiCompatibleApiBase: "https://cdn.12ai.org/v1",
  geminiNativeApiBase: "https://generativelanguage.googleapis.com/v1beta",
...
2. Upload one PDF, choose the request format / model API base / models, and choose the image placeholder strategy (`source-crop`, `gemini`, `auto`, or `off`)
...
- per-job request settings are written to `runtime_config.json` under the job workspace for execution
- user-entered API keys are not stored in browser localStorage and are redacted from the saved runtime config after the job finishes
