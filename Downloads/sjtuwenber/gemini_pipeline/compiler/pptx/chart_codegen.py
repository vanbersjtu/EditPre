...
def _is_google_endpoint(base_url: Optional[str]) -> bool:
...
def _build_google_generate_url(base_url: str, model: str) -> str:
...
def generate_chart_code(
    ...
    is_google_endpoint = _is_google_endpoint(use_base_url)
    if api_key:
        key = api_key
    ...
    try:
        ...
        if is_google_endpoint:
            ...
            url = _build_google_generate_url(use_base_url, use_model)
            ...
            code = _extract_google_text(data)
        else:
            request_body = {
                "model": use_model,
                "messages": [
            ...
            url = f"{use_base_url.rstrip('/')}/chat/completions"
            payload = json.dumps(request_body, ensure_ascii=False).encode("utf-8")
            req = urllib.request.Request(
                url=url,
                data=payload,
        ...
        if not code:
            raise RuntimeError("Model returned empty chart code")
        if code.startswith("```"):
            lines = code.split("\n")
